...
// Legacy slot that already exists in db20.js and has working sprite tiles.
const LEGACY_PATCHES = [
  { key: "MUSKETER", index: 19 },
];
...
let nextId = 19200;
const sets: unknown[] = [];
const legacySetPatches: unknown[] = [];
...
function buildSet(src: any, key: string) {
...
for (const key of NEW_KEYS) {
  ...
  sets.push(buildSet(src, key));
}

for (const patch of LEGACY_PATCHES) {
  const src = raw.sets[patch.key];
  if (!src) throw new Error("missing set " + patch.key);
  const built = buildSet(src, patch.key);
  legacySetPatches.push({
    index: patch.index,
    level: built.level,
    key: built.key,
    name: built.name,
    race: built.race,
    slots: built.slots,
    setTitle: built.setTitle,
    oldSetTitle: built.oldSetTitle,
    setBonusId: built.setBonusId,
    oldSetBonusId: built.oldSetBonusId,
  });
}

const runtime = await Deno.readTextFile(RUNTIME);
const payload = JSON.stringify({ effects, sets, legacySetPatches, startHint: 49 });
